...
`getResult(tags, results)`는 사용자가 누적한 태그와 각 결과의 `tags` 배열의 교집합 점수로 순위를 매긴다. **q1(맛) 태그는 ×2 가중치**, q2(멘탈)·q3(예산)는 ×1 — 맛이 1차 기준, 멘탈·예산은 tie-break(plan.md Phase 2.1). 최고 점수 결과를 반환하고, 동점이면 무작위 선택, **점수 0이면 `fallback`** 을 반환한다.

설계 함의:
- 결과의 `tags`는 보통 3개(맛 1 + 멘탈 1 + 예산 1). "이 조합의 사람"을 정의한다. 예산 2개를 한 결과가 커버하려면(4태그) 해당 맛+멘탈 조합의 나머지 예산 1개를 다른 결과가 전담해야 동점이 안 생긴다(아래 "완전 결정론적 설계" 참고).
- **현재 데이터는 완전 결정론적으로 설계됨(Phase 2.3, 2026-07-25 완료).** 맛×멘탈 9조합마다 결과 2개(예산 2개 커버하는 "넓은" 결과 + 1개만 커버하는 "좁은" 결과)를 배정해 27경로(맛3×멘탈3×예산3) 전부가 겹침 없이 고유 승자를 가진다. 결과 추가/수정 시 이 원칙을 깨지 않도록 반드시 27경로 재시뮬레이션한다.
- 결과 개수는 현재 18개(+fallback).
...
**판정 기준:** 동점이 하나라도 나오거나(같은 답변에 매번 다른 메뉴 = 설계 원칙 위반) 특정 결과로 쏠리거나 의도치 않은 fallback이 다수면 태그를 조정한다. 현재 기준선: 27경로, fallback 0, 동점 0, 최대 쏠림 2/27.
