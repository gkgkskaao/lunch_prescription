...
> 단일 서비스 구조라 원본 하네스의 page-builder 단계(업종별 HTML 생성)는 없다. HTML은 `index.html` 하나로 고정. 이 오케스트레이터는 결과 풀 확장(plan.md Phase 2.2·2.3에서 3→12→18개로 확장한 이력)에서 쓴다. 현재 데이터는 27경로 완전 결정론적 설계(동점 0)이므로, 결과 추가 시 태그가 기존 결과와 겹쳐 동점이 생기지 않는지 반드시 재시뮬레이션한다.
