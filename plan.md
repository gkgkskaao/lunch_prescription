...
> 현재 상태: Phase 0·2 완료, Vercel 배포 중(https://lunch-prescription.vercel.app). Phase 1은 GA4 측정 ID만 잔여.
...
> **다음 세션 시작점 (2026-09-11 종료 시)**: 이 앱은 1단계 미착수. GA4 측정 ID 발급(사용자) 대기, `og:image` 절대경로 수정(#4)은 승인 대기. 우선순위는 3sec 가게 리스트가 먼저.
...
| 7 | `og:image`가 상대경로, `og:url` 없음 → 링크 직접 붙여넣기 시 썸네일 누락 가능 | 절대 URL로 수정 + `og:url` 추가 | ⏳ 보조 증거 스프린트 1단계 |
...
| # | 할 일 | 담당 |
|---|---|---|
| 1 | GA4 속성 생성 → 측정 ID(`G-XXXX`) 발급 | 사용자 |
| 2 | `index.html`에 gtag 스니펫 추가 → 운영 환경 이벤트 수신 확인 | Claude |
| 3 | GA4에서 `share_click`·`image_save`·`result_view`를 주요 이벤트로 지정 | 사용자 |
| 4 | `og:image` 절대 URL 수정 + `og:url` 추가 (이슈 #7) | Claude |
...
- 채널은 **2개만**: 지인 단톡방 3~5곳 + 블라인드 1회. 채널별 UTM 링크 분리(`?utm_source=kakao_friends`, `?utm_source=blind`).
...
```
Phase 0 ✅ ──▶ Phase 1 ✅(GA4 잔여) ──▶ Phase 2 ✅
                                          │
                                          ▼
...
## 6. 즉시 착수할 작업 (2026-09-11 기준)

1. `og:image` 절대 URL + `og:url` 추가 (보조 증거 스프린트 1단계 #4).
2. GA4 측정 ID 발급 → gtag 스니펫 연결 → 실기기 이벤트 수신 확인 (#1~3).
3. 지인 단톡방·블라인드용 UTM 링크 준비 후 2026-09-14부터 가벼운 시딩.
